      ...
      <div className="container py-20 md:py-28">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-6">
            ...
            <h2 className="font-display text-3xl md:text-4xl font-bold leading-snug text-primary-foreground">
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              {features.map(item => (
                <div key={item.label} className="flex items-start gap-3 p-4 rounded-xl bg-primary-foreground/[0.05] border border-primary-foreground/[0.08]">
                  ...
                  <div>
                    <span className="font-body text-sm font-semibold text-primary-foreground/90 block">{item.label}</span>
                    <span className="text-xs text-primary-foreground/40 font-body">{item.desc}</span>
                  </div>
                </div>
              ))}
            </div>
