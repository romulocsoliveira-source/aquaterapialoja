        ...
        {["Todos", ...CHANNELS].map(ch => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-body transition-colors ${
              channelFilter === ch ? "bg-accent text-accent-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"
            }`}
          >
            {ch !== "Todos" && channelIcons[ch as Channel]}
            {ch}
          </button>
        ))}
      </div>
              ...
              {filtered.map(o => (
                <tr key={o.id} className="border-b border-border/50 hover:bg-secondary/30">
                  ...
                  <td className="p-3 hidden md:table-cell">
                    <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full ${channelColors[o.channel]}`}>
