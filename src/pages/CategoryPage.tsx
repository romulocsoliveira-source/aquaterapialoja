    ...
    return (
      <div className="bg-background min-h-screen">
        <div className="container py-8 md:py-10">
          <div className="mb-6">
            <div className="h-7 w-44 bg-secondary rounded-lg animate-pulse" />
            <div className="h-4 w-28 bg-secondary rounded-lg animate-pulse mt-2" />
          </div>
    ...
    <>
      ...
      <div className="bg-background min-h-screen">
        ...
        <div className="container py-4 md:py-8">
          ...
          {/* Toolbar */}
          <div className="flex flex-wrap items-center gap-2.5 mb-6 p-3.5 bg-card rounded-xl border border-border">
            <button
              onClick={() => setShowFilters(!showFilters)}
              className={`flex items-center gap-2 text-sm font-body px-3.5 py-2 rounded-lg border transition-all duration-200 ${showFilters ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:text-foreground hover:border-foreground/20"}`}
            >
            ...
            <div className="relative ml-auto">
              <select
                ...
                className="appearance-none bg-background text-foreground text-sm font-body px-3.5 py-2 pr-8 rounded-lg border border-border focus:outline-none focus:ring-2 focus:ring-primary/30 cursor-pointer transition-all duration-200"
              >
            ...
            <div className="flex flex-wrap gap-2 mb-5 animate-fade-in">
              {[
                ...
                <button
                  ...
                  className={`text-xs font-body px-3.5 py-2 rounded-full border transition-all duration-200 ${priceFilter === f.value ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:text-foreground hover:border-foreground/20 bg-card"}`}
                >
                  {f.label}
                </button>
              ))}
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16">
              <Package size={44} className="mx-auto text-muted-foreground/30 mb-3" />
              <p className="text-muted-foreground font-body text-base">Nenhum produto encontrado nesta categoria.</p>
              <Link to="/" className="inline-flex items-center gap-2 text-primary font-body font-medium mt-3 hover:underline">
                Voltar ao início
              </Link>
            </div>
          )}
