...
export default function PDVPage() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "F4") { e.preventDefault(); if (cart.length > 0 && !lastSale) setShowPayment(true); }
      if (e.key === "Escape") {
  ...
  const findProductInCache = (code: string): Product | undefined => {
    ...
    let found = products.find(p => p.barcode === clean);
    if (found) return found;
    const stripped = clean.replace(/^0+/, "");
    found = products.find(p => p.barcode.replace(/^0+/, "") === stripped);
    if (found) return found;
    found = products.find(p => p.barcode.includes(clean) || clean.includes(p.barcode));
    if (found) return found;
    found = products.find(p => p.sku === clean);
    return found;
  };
  ...
  const mapDBProduct = (p: any): Product => ({
    ...
    category: p.category, categorySlug: p.category_slug,
    parentCategory: p.parent_category || undefined,
    image: p.image || "", images: p.images || [],
    description: p.description || "", benefits: p.benefits || [],
    specs: p.specs || [], instructions: p.instructions || undefined,
    rating: Number(p.rating) || 0, reviews: p.reviews || 0,
    badge: p.badge || undefined,
    variations: (p.variations || []).length > 0 ? p.variations : undefined,
    isNew: p.is_new || false, isBestSeller: p.is_best_seller || false,
    sku: p.sku, barcode: p.barcode, stock: p.stock || 0,
    costPrice: p.cost_price ? Number(p.cost_price) : 0,
  });

  const findProductInDB = async (code: string): Promise<Product | undefined> => {
    const clean = code.replace(/\D/g, "").trim();
    if (!clean) return undefined;
    console.log("[PDV] Cache miss, buscando no banco:", clean);
    ...
    if (error || !data || data.length === 0) {
      const { data: flexData } = await supabase
        ...
        .ilike("barcode", `%${clean}%`)
        .limit(1);
      if (!flexData || flexData.length === 0) return undefined;
      return mapDBProduct(flexData[0]);
    }
  ...
  const handleBarcode = async (code?: string) => {
    ...
    if (!bc) return;
    console.log("[PDV] Buscando barcode:", bc, "| Produtos em cache:", products.length);
    let found = findProductInCache(bc);
    if (!found) {
      found = await findProductInDB(bc);
      if (found) refetchProducts();
    }
    if (found) {
      addToCart(found);
      ...
      const suggestionText = suggestions?.length
        ? `\nProdutos disponíveis: ${suggestions.map(s => s.name).join(", ")}`
        : "";
      toast.error(`Produto não encontrado para código: ${bc}${suggestionText}`);
    }
    setBarcodeInput("");
  };

  const handleCameraScan = async (code: string) => {
    console.log("[PDV] Camera scan:", code);
    ...
    receiptWindow.document.write(`
      ...
      <style>
        ...
        ${sale.items.map((i, idx) => `
          <div style="margin:6px 0"><div>${idx + 1}. ${i.product.name}</div>
          <div class="item"><span>${i.quantity}x ${formatPrice(i.product.promoPrice || i.product.price)}</span>
          <span>${formatPrice((i.product.promoPrice || i.product.price) * i.quantity)}</span></div></div>
        `).join("")}
        <div class="line"></div>
  ...
  const finalizeSale = async (method: string) => {
    ...
    if (method === "cash" && !showCashInput) { setShowCashInput(true); return; }
    setProcessing(true);
    try {
      const { data: order, error: orderError } = await supabase.from("orders").insert({
        user_id: user.id, total, payment_method: method, status: "delivered", shipping_cost: 0, discount: discountAmount,
      }).select("id").single();
      ...
      const orderItems = cart.map(i => ({
        ...
        product_image: i.product.image, quantity: i.quantity, unit_price: i.product.promoPrice || i.product.price,
      }));
      ...
      await supabase.from("financial_transactions").insert({
        type: "income", category: "Venda PDV",
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="bg-secondary border-b border-border">
        <div className="container flex items-center justify-between py-3">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" onClick={loadHistory} className="gap-1 text-xs"><Clock size={14} /> Vendas do Dia</Button>
            {cart.length > 0 && (
              <Button variant="outline" size="sm" onClick={() => setShowLabels(true)} className="gap-1 text-xs"><Tag size={14} /> Etiquetas</Button>
            )}
            ...
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="w-full max-w-sm mx-auto">
              <div className="bg-white text-black rounded-xl shadow-2xl overflow-hidden" id="receipt-content">
                ...
                <div className="px-4 py-2">
                  <div className="flex justify-between text-[10px] font-bold text-gray-500 mb-1"><span>ITEM</span><span>TOTAL</span></div>
                ...
                <div className="px-4 py-2 space-y-1">
                  ...
                  {lastSale.discount > 0 && (
                    <div className="flex justify-between text-xs text-red-600"><span>Desconto</span><span>-{formatPrice(lastSale.discount)}</span></div>
                  )}
                  <div className="flex justify-between text-base font-bold pt-1"><span>TOTAL</span><span>{formatPrice(lastSale.total)}</span></div>
                </div>
                <div className="mx-4 border-t border-dashed border-gray-300" />
                <div className="px-4 py-2 space-y-1">
                  <div className="flex justify-between text-xs">
                  ...
                  {lastSale.method === "cash" && lastSale.cashReceived > 0 && (
                    <>
                      <div className="flex justify-between text-xs"><span>Valor Recebido</span><span>{formatPrice(lastSale.cashReceived)}</span></div>
                      <div className="flex justify-between text-xs font-bold"><span>Troco</span><span>{formatPrice(lastSale.change)}</span></div>
                    </>
                  )}
                </div>
                <div className="mx-4 border-t border-dashed border-gray-300" />
              ...
              <div className="flex flex-wrap gap-3 justify-center mt-6">
                ...
                <Button variant="outline" className="gap-2 bg-card" onClick={() => setShowNFe(true)}>
                  <Receipt size={16} /> Emitir Nota Fiscal
                </Button>
              ...
              {lastSale && (
                <NFeSimulator open={showNFe} onOpenChange={setShowNFe} sale={lastSale} customerName={customerName} />
              )}
            </motion.div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {showHistory && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 bg-background/80 z-40 flex items-center justify-center" onClick={() => setShowHistory(false)}>
      ...
      <div className="flex-1 container py-4 grid md:grid-cols-[320px,1fr] gap-4">
        ...
        <div className="space-y-4">
          ...
          <BarcodeScanner open={showCameraScanner} onOpenChange={setShowCameraScanner}
            onScan={(code) => { handleCameraScan(code); setTimeout(() => setShowCameraScanner(true), 800); }}
            title="Escanear Produto — PDV" />
          <ProductLabelPrint open={showLabels} onOpenChange={setShowLabels}
        ...
        <div className="bg-card border border-border rounded-xl flex flex-col h-[calc(100vh-180px)]">
          <div className="p-4 border-b border-border flex items-center justify-between">
            <h2 className="font-body font-semibold flex items-center gap-2"><ShoppingCart size={18} /> Venda ({totalItems} itens)</h2>
            {cart.length > 0 && (
              <button onClick={() => { setCart([]); setDiscountPercent(0); setDiscountFixed(0); }} className="text-xs text-muted-foreground hover:text-destructive">Limpar</button>
            )}
          </div>
          ...
          <div className="border-t border-border p-4 space-y-3">
                ...
                {showDiscount && (
                  <div className="flex gap-2 mt-2">
                    <div className="flex-1">
                      ...
                      <input type="number" min="0" max="100" value={discountPercent || ""} onChange={e => { setDiscountPercent(Number(e.target.value)); setDiscountFixed(0); }}
                        className="w-full bg-secondary text-foreground px-3 py-1.5 rounded text-sm focus:outline-none focus:ring-1 focus:ring-accent/50" placeholder="0" />
                    </div>
                    <div className="flex-1">
                      <label className="text-[10px] text-muted-foreground">R$ Desconto</label>
                      <input type="number" min="0" step="0.01" value={discountFixed || ""} onChange={e => { setDiscountFixed(Number(e.target.value)); setDiscountPercent(0); }}
                        className="w-full bg-secondary text-foreground px-3 py-1.5 rounded text-sm focus:outline-none focus:ring-1 focus:ring-accent/50" placeholder="0.00" />
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-1">
              ...
              {discountAmount > 0 && (
                <div className="flex justify-between text-sm text-green-400"><span>Desconto</span><span>-{formatPrice(discountAmount)}</span></div>
              )}
              ...
              <div className="space-y-2">
                ...
                {!showCashInput && (
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { method: "pix", icon: <QrCode size={20} className="text-accent" />, label: "Pix" },
                      { method: "credit_card", icon: <CreditCard size={20} className="text-accent" />, label: "Crédito" },
                      { method: "debit_card", icon: <CreditCard size={20} className="text-accent" />, label: "Débito" },
                      { method: "cash", icon: <Banknote size={20} className="text-accent" />, label: "Dinheiro" },
                    ].map(pm => (
                      <button key={pm.method} onClick={() => finalizeSale(pm.method)} disabled={processing}
                        ...
                        {pm.icon}
                        <span className="text-xs font-body">{pm.label}</span>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {!showCashInput && (
                  <button onClick={() => setShowPayment(false)} className="w-full text-xs text-muted-foreground hover:text-foreground py-1" disabled={processing}>Cancelar</button>
                )}
              </div>
            )}
          </div>
